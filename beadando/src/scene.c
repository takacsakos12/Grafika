...
/* Veletlen lebegopontos szamot ad vissza a megadott intervallumban. */
static float random_float(float min, float max)
{
    return min + ((float)rand() / (float)RAND_MAX) * (max - min);
}

/* Egy erteket a megadott minimum es maximum koze szorit. */
...
/* Egyszeru teglatestet rajzol adott meretekkel. */
...
/* Kirajzol egy texturazott statikus OBJ modellt a palyan. */
...
/* Uj teglalap alaku collidert ad a palyahoz. */
...
/* Vizszintes falat hoz letre ket x koordinata kozott. */
...
/* Fuggoleges falat hoz letre ket z koordinata kozott. */
...
/* Megnezi, hogy egy kor alaku objektum metszi-e a teglalap collidert. */
...
/* Megnezi, hogy egy szakasz metszi-e a teglalap collidert. */
...
/* Megnezi, hogy van-e fal a dron es a jatekos kozott. */
...
/* Uj crate objektumot ad a scene objektumlistajahoz. */
...
/* Uj plant capsule objektumot ad a scene objektumlistajahoz. */
...
/* Uj aktivalhato generatort ad a palyahoz. */
...
/* Belso ajtot ad a palyahoz sajat colliderrel. */
...
/* Alaphelyzetbe allitja a reszecskerendszer elemeit. */
...
/* Letrehoz egy uj szikra reszecsket a megadott pozicioban. */
...
/* Beallitja a generatorokhoz tartozo szikraforrasok helyet. */
...
/* Frissiti a szikra reszecskek kibocsatasat es mozgasat. */
...
/* Kirajzolja az aktiv szikra reszecskeket. */
...
/* Texturazott falat rajzol collider meretek alapjan. */
...
/* Kirajzolja az exit ajtot es annak nyilo paneljet. */
...
/* Kirajzol egy generatort, aktiv allapotban mas szinezessel. */
...
/* Kirajzolja a belso ajtot zsaner koruli forgatassal. */
...
/* Alaphelyzetbe allit egy dront az utvonal kezdopontjara. */
...
/* Mozgatja a dront az aktualis waypoint fele. */
...
/* Megnezi, hogy a dron latoszogben es tavolsagon belul latja-e a jatekost. */
...
/* Kirajzolja a dron latomezot jelzo piros szektort. */
...
/* Kirajzolja a dronok utvonalat segedvonalkent. */
...
/* Egyszeru dobozokbol kirajzolja a dron modellt. */
...
/* Megnezi, hogy a jatekos elerte-e a kijaratot a nyitott exit ajto utan. */
static bool is_player_at_exit_internal(const Scene* scene, float player_x, float player_z)
{
    float exit_x = 3.0f;
    float exit_z = 19.0f;
...
void interact_scene(Scene* scene, float player_x, float player_z)
{
    ...
    for (i = 0; i < scene->generator_count; ++i) {
        ...
        if (distance_squared <= interaction_radius * interaction_radius) {
            ...
            printf("Generator activated: %d/%d\n", scene->active_generator_count, scene->generator_count);
...
void update_scene(Scene* scene, double delta_time, float player_x, float player_z)
{
    ...
    if (is_player_at_exit_internal(scene, player_x, player_z)) {
        scene->game_won = true;
...
{
    return is_player_at_exit_internal(scene, player_x, player_z);
}
